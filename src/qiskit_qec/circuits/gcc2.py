...
class GrossC2(CodeCircuit):
    ...
    def _perform_syndrome_measurements(self, qc, qregs, state):
        for t in range(self.T):
            ...
            self.perform_syndrom_cycle(qc, qregs, t, state)
    
    #TODO Finish this cycle again and then we can see what happens
    def perform_syndrom_cycle(self, qc, qregs, t, state):
        creg_x = ClassicalRegister(len(self.x_gauges), name="round_" + str(t) + "_x_bits")
        qc.add_register(creg_x)
        creq_z = ClassicalRegister(len(self.z_gauges), name="round_" + str(t) + "_z_bits")
        qc.add_register(creq_z)
...
if __name__ == "__main__":
    code = GrossCode()
    circuit = GrossC2(code, T=3, basis="x", noise_model=(0.01, 0.01), round_schedule="xz")